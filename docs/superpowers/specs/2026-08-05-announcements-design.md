...
| 管理者側 | 管理画面の左メニューに「お知らせ」を追加。作成・編集・公開終了・確認状況の表示 |
...
### 管理者: お知らせ画面（`app/components/AdminAnnouncementsSection.tsx`）

管理画面の左メニューに独立した項目として追加する。管理設定内のカードにすると
巨大なメンバー表の下に埋もれるため、1クリックで開ける独立セクションにする。

CLAUDE.md §5 の「新セクション追加の4点セット」に従う:
`AdminSection` 型に `"announcements"` を追加 → `navItems` に `{ id: "announcements", label: "お知らせ" }`
→ `AdminNavIcon` にインラインSVGのアイコンを追加 → `{adminSection === "announcements" && (...)}` ブロックを追加。
表示位置は「休眠メンバー」の下、管理者限定項目の上。
...
- **メニューのバッジ**: 未確認者が残っている公開中の必読お知らせの件数を、
  既存の `invoiceMissingNavCount` と同じ体裁でメニュー項目に表示する
